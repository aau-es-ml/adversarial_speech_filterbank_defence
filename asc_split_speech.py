...
from asc_utilities.zht_dataset_utilities import get_normal_adv_wav_files
from external.my_rvadfast_py import speechproc
...
def speech_silence_split(file_name, speech_save_file_name, silence_save_file_name):
  ...
  assert len(voice_activity_mask) >= len(data) - 160, f'{len(voice_activity_mask)}, {len(data)}'
...
if __name__ == '__main__':
  from experiment_config import DATA_ROOT_PATH, DATA_ROOT_SPLITS_PATH, PROJECT_APP_PATH

  def main(root: pathlib.Path = DATA_ROOT_PATH):
    ...
    for data_s in [
      ...
      normal_files, adv_files = get_normal_adv_wav_files(root / f'adversarial_dataset-{data_s}')

      print(len(normal_files), len(adv_files))

      out_root = ensure_existence(DATA_ROOT_SPLITS_PATH / data_s)
      out_dir_speech = out_root / "speech"
      out_dir_silence = out_root / "silence"

      if parallel:
        parallel_umap(compute_split,
                      normal_files,
                      func_kws=dict(data_s=data_s,
                                    adv_files=adv_files,
                                    out_dir_speech=out_dir_speech,
                                    out_dir_silence=out_dir_silence))
      else:
        for normal_f in tqdm.tqdm(normal_files):
          compute_split(normal_f, packed=(data_s, adv_files, out_dir_speech, out_dir_silence))
