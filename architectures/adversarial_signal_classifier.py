...
class AdversarialSignalClassifier(nn.Module):
  ...
  def __init__(self, in_channels: int, in_height: int, in_width: int, article_check: bool = False):
    super().__init__()
    if article_check:
      assert in_channels == 1, f'was {in_channels}'
      assert in_height == 40, f'was {in_height}'
      assert in_width == 31, f'was {in_width}'
    ...
    self.convs = nn.Sequential(nn.Conv2d(1,
                                         64,
        ...
        max_pool2d_hw_shape(
            conv2d_hw_shape(
                max_pool2d_hw_shape(
                    conv2d_hw_shape(
                        max_pool2d_hw_shape(
                            conv2d_hw_shape(self._in_shape[1:],
                                            conv_kernel,
                                            conv_stride),
                            max_pool_1_kernel),
                        conv_kernel),
