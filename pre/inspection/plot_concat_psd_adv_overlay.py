            ...
            for ko, a in plots.items():
                with FigureSession():
                    ...
                    for k, (b, sr, a_p, id_a) in a.items():
                        ...
                        asd.append(numpy.sqrt(spectrum))

                    ax1 = pyplot.gca()
                    ax2 = ax1.twinx()
                    ax2.plot(
                        f,
                        numpy.abs(reduce(numpy.subtract, asd)),
                        color="0.6",
                        label="diff",
                    )
                    fix_edge_gridlines()
                    ax1.legend(loc="upper center")
                    ax2.set_ylabel("Absolute energy", color="0.6")

                    pyplot.title(f"{p.stem}")
                    save_embed_fig(f"{p}_ltass.pdf")
